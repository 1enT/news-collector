	...
	def find_last_news(self):
		...
		for i, each_new in enumerate(list_of_news):
			...
			if True:
				...
				# 	break
				# self.last_news.append({"source": "Татар-информ", "time": time, "title": title, "lead": lead, "text": text, "link": link, "img_url": img_url})
				print({"source": "Татар-информ", "time": time, "title": title, "lead": lead, "text": text, "link": link, "img_url": img_url})
				print()
			else:
				break
