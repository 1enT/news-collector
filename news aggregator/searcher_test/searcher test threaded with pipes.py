...
def process_keywords_parts(num, text, words, conn):
	# print(text)
	# exit()
	for keyword in words:
		...
		for i in range(0, len(text) - keyword_len + 1):
			...
			if dist/len(slice_of_text) <= deviation:
				...
				conn.send([True, f"{num} | {keyword} | {slice_of_text} | {str(dist)}"])
				conn.close()
	conn.send([False, ""])
	conn.close()
...
if __name__ == '__main__':
	...
	threads = [
				Process(target=process_keywords_parts, args=(i+1, text, keyword_parts[i], child_conn))
				for i in range(process_count)
			]
